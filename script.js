...
function displayTasksFromStorage() {
  ...
  let htmlStr = "";
  let myTaskStorage = JSON.parse(window.localStorage.getItem('mytasks'));
  if (myTaskStorage) {
      for(let i=0; i < myTaskStorage.length; i++){
      htmlStr = `
      <div class="card border-info mt-2">
          <div id ="${myTaskStorage[i].ID}" class="card">
              <div class="card-header">
                  <div class="row">
                      <div class="col-md-8">
                          <h5 class="card-title">${myTaskStorage[i].TaskName}</h5>
                      </div>
                      <div class="col-md-4">
                          <h5 class="dueDateLabel">Due Date: ${myTaskStorage[i].DueDate}</h5>                        
                      </div>
                  </div>
              </div>
              <div class="card-body">
                  ...
                  <div class="overflow-auto border">
                      <p class="card-text">${myTaskStorage[i].TaskDesc}</p>
                  </div>
                  <div class="row mt-5">
                      <div class="col-md-6 border">
                          ...
                          <p>${myTaskStorage[i].AssignedTo}</p>
                      </div>
                      <div class="col-md-6 border">
                          <label for="selectStatus">
                              <dt>Status</dt>
                          </label>
                          <p>${myTaskStorage[i].Status}</p>
                      </div>
                  </div>
              </div>
